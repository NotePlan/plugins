...
import {   getNonNegativeSearchTermsFromPluginExtendedSyntax,
noteAndLineIntersection, normaliseSearchTerms, optimiseOrderOfSearchTerms, applyPluginSearchOperators, validateAndTypeSearchTerms } from '../src/pluginExtendedSyntaxHelpers'
...
describe('pluginExtendedSyntaxHelpers.js tests', () => {
  ...
  describe('applyPluginSearchOperators() from pluginExtendedSyntaxHelpers.js -> resultObjectType', () => {
    ...
    test('should return no results from simple no results', () => {
      ...
      const expectedNoteBasedOutput: resultOutputV3Type = {
        // for no results
        // searchTermsRepArr: ['TERM1'],
        searchTermsStr: 'TERM1',
        searchOperatorsStr: '',
        searchTermsToHighlight: ['TERM1'],
        resultNoteAndLineArr: [],
        resultCount: 0,
        fullResultCount: 0,
        resultNoteCount: 0,
      }
      const result = applyPluginSearchOperators(combinedResults)
    ...
    test('should return no results from [TERM2 -TERM2] search', () => {
      ...
      const expectedNoteBasedOutput: resultOutputV3Type = {
        ...
        searchTermsStr: 'TERM2 -TERM2',
        searchOperatorsStr: '',
        searchTermsToHighlight: ['TERM2'],
        resultNoteAndLineArr: [],
        resultCount: 0,
        fullResultCount: 0,
        resultNoteCount: 0,
    ...
    test('should return narrower !term results', () => {
      ...
      const expectedNoteBasedOutput: resultOutputV3Type = {
        ...
        searchTermsStr: 'TERM1 !TERM2 +TERM3',
        searchOperatorsStr: '',
        searchTermsToHighlight: ['TERM1', 'TERM3'],
        resultNoteAndLineArr: [
          { noteFilename: 'file5', line: '5.1 includes TERM1', index: 0 },
          { noteFilename: 'file5', line: '5.2 includes TERM3', index: 0 },
          { noteFilename: 'file7', line: '7.1 (W£%&W(*%&)) TERM1', index: 0 },
    ...
    test('should return wider -term results', () => {
      ...
      const expectedLineBasedOutput: resultOutputV3Type = {
  ...
  describe('getNonNegativeSearchTermsFromPluginExtendedSyntax', () => {
    test('should return empty array from empty string', () => {
      const result = getNonNegativeSearchTermsFromPluginExtendedSyntax([''])
      expect(result).toEqual([])
    })
    test('should return empty array from empty strings', () => {
      const result = getNonNegativeSearchTermsFromPluginExtendedSyntax(['', ''])
      expect(result).toEqual([])
    })
    test('should return empty array from mixed input 1', () => {
      const result = getNonNegativeSearchTermsFromPluginExtendedSyntax(['Holy','Spirit','-odd','!evil'])
      expect(result).toEqual(['Holy','Spirit'])
    })
    test('should return array with all terms from mixed positive input', () => {
      const result = getNonNegativeSearchTermsFromPluginExtendedSyntax(['Holy','Spirit','+odd','+even'])
