 ...
 */
export function getPreviousNPPeriodString(periodStringIn: string, periodType: string): string {
  try {
    const trimmed = periodStringIn.trim()
    const wantCompactQuarter = /^(\d{4})Q([1-4])$/i.test(trimmed)
    let periodString = trimmed
    ...
    const interval = intervalByType[periodType]
    if (!interval) {
      logError('dateTime / getPreviousNPPeriodString', `Unknown periodType '${periodType}'`)
      return ''
    }
    const newDate = calcOffsetDate(periodString, interval)
    if (!newDate) {
    ...
    if (periodString.match(RE_ISO_DATE)) {
      ...
      logError('dateTime / getPreviousNPPeriodString', `Unrecognized period string '${periodString}'`)
      return ''
    }
    let out = moment(newDate).format(momentDateFormat)
    if (wantCompactQuarter && momentDateFormat === MOMENT_FORMAT_NP_QUARTER) {
    ...
    logError('dateTime / getPreviousNPPeriodString', e.message)
    return ''
  }
}
