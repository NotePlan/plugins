...
export function generateCSSFromTheme(themeNameIn: string = ''): string {
  try {
    ...
    if (styleObj) {
      ...
      rootSel.push(`--fg-ok-color: color-mix(in oklch, var(--fg-main-color), green 30%)`)
    ...
    // Lozenge: fill with timeBlockColor, text uses main background (FG/BG swapped vs former plain colored text)
    ...
    tempSel = []
    tempSel.push(`font-size: 84%`)
    tempSel.push(`color: var(--bg-main-color)`)
    tempSel.push(`background-color: rgb(from var(--fg-timeBlock) r g b / 0.6)`)
    tempSel.push('border-radius: 5px')
    tempSel.push('padding-inline: 3px')
    output.push(makeCSSSelector('.timeBlock', tempSel))

    // Scheduled >date lozenge: tint fill, main background text (same shape as .timeBlock)
    tempSel = []
    tempSel.push(`font-size: 84%`)
    tempSel.push(`color: var(--tint-color)`)
    tempSel.push(`background-color: rgb(from var(--tint-color) r g b / 0.2)`)
    tempSel.push('border-radius: 5px')
    tempSel.push('padding-inline: 3px')
