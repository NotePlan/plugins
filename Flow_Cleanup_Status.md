...
Branch: `flow-cleanup` · 33 commits · not pushed
...
| | Start | Now | Change |
|---|---|---|---|
| Raw `flow check` errors | 8,254 | **867** | −89% |
| Unique `file:line:code` sites | 1,314 | **525** | −60% |
...
diffed. **101 of 126 changed source files emit byte-identical JavaScript.** The `jgclark.Dashboard`
and `dwertheimer.TaskAutomations` plugin builds also produce byte-identical `script.js`.

The 25 files that do differ break down as follows.
...
## What the remaining 525 sites are
...
The purely-mechanical categories are done: `missing-local-annot`, `missing-empty-array-annot`,
`underconstrained-implicit-instantiation`, `signature-verification-failure`, `method-unbinding`,
`definition-cycle` and `value-as-type` are all at or near zero.

**What's left needs either a code edit or a design decision**, and all of it is written up in
[Flow_Human_Review.md](./Flow_Human_Review.md) — 21 items, graded 🔴 breakage / 🟠 silently wrong /
