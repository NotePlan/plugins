...
import { getDefaultConfiguration, getOrMakeConfigurationSection } from '../../nmn.Templates/src/configuration'
...
export async function periodStats(): Promise<void> {
  ...
  for (const elem of hSumTotals) { // .entries() implied
  ...
  for (const elem of hCounts) { // .entries() implied
    const hashtagString = (pref_showAsHashtagOrMention) ? elem[0] : elem[0].slice(1)
  ...
  // First process more complex 'SumTotals', calculating appropriately
  for (const elem of mSumTotals) { // .entries() implied
...
async function removeSection(note: TNote, heading: string): number {
  ...
  let existingHeadingIndex: number
...
function calcHashtagStatsPeriod(fromDateStr, toDateStr):
  ...
  console.log(`\thashtagsToIgnore: ${hashtagsToIgnore}`)
...
function calcMentionStatsPeriod(fromDateStr, toDateStr):
  ...
  console.log(`\tmentionsToIgnore: ${mentionsToIgnore}`)
