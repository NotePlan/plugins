...
for (const [plugin, filename] of [
  ...
  const mock = `<script>
    ...
    if (previewParams.has('dense')) {
      for (let day=14;day<21;day++) {
        for (let index=0;index<12;index++) previewEvents.push({...previewEvent,id:'all-'+day+'-'+index,title:'Planning '+(index+1),date:new Date(2026,8,day).toISOString(),endDate:new Date(2026,8,day+1).toISOString(),isAllDay:true,isRecurring:false});
        previewEvents.push({...previewEvent,id:'timed-'+day,title:'Morning meeting',date:new Date(2026,8,day,8).toISOString(),endDate:new Date(2026,8,day,9).toISOString(),isRecurring:false});
      }
    }
    window.DataStore={preference:async()=>Number(previewParams.get('firstDay') || 2)};
    window.Calendar={availableCalendars:async()=>[{id:'work',title:'Work',source:'iCloud',color:'#d96512',isWritable:true,isEnabled:true},{id:'personal',title:'Personal',source:'iCloud',color:'#4488bb',isWritable:true,isEnabled:true}],eventsBetween:async()=>previewEvents,eventEditorCapabilities:async()=>new URLSearchParams(location.search).has('legacy')?null:({version:1}),eventForEditing:async()=>({...previewEvent}),eventByID:async()=>({...previewEvent}),saveEvent:async(data)=>({...data,id:'preview'}),removeEvent:async()=>true};
  ...
  const browse = filename === 'calendar.html'
    ? "state.viewDate=new Date(2026,8,14);switchView(new URLSearchParams(location.search).get('view'));"
    : "layoutMode='fixedWeek';renderCalendar();updateCalendarDisplay();"
  html = html.replace('</body>', `<script>setTimeout(function(){if(new URLSearchParams(location.search).has('view')){${browse}}else if(new URLSearchParams(location.search).has('new')){${newEvent}}else{${show}}},600);</script></body>`)
