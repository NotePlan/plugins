...
### Fixed
- dev: removed a misplaced `openCalendarNoteInSplit` command object from `plugin.settings` that caused `plugin.settings[13] has no valid key; skipping` on every settings update. The command remains correctly registered in `plugin.commands`. (Closes #765; thanks @dwertheimer)
