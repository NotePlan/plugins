...
import { getTodaysReferences, findOpenTodosInNote } from '@np/helpers/NPnote'
import { showMessage } from '@np/helpers/userInput'
import { isTimeBlockLine } from '@np/helpers/timeblocks'
...
import { insertContentUnderHeading } from '@np/helpers/NPParagraph'
  ...
  if (list && list.length > 0 && note) {
    ...
    if (shouldFold && heading !== '') {
      ...
      if (thePara) {
        ...
        // @ts-ignore - the function is not being removed from the Editor object.
        if (Editor.isFolded) {
