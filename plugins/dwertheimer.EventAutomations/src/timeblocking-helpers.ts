...
import { getDateObjFromDateTimeString, getTimeStringFromDate, removeDateTagsAndToday, removeRepeats } from '@np/helpers/dateTime'
import { sortListBy } from '@np/helpers/sorting'
import { textWithoutSyncedCopyTag } from '@np/helpers/syncedCopies'
import { createPrettyLinkToLine, createWikiLinkToLine } from '@np/helpers/NPSyncedCopies'
import { logError, JSP, copyObject, clo, clof, logDebug, logWarn } from '@np/helpers/dev'
...
// @ts-ignore - can't find a Flow type for RegExp
...
export function matchTasksToSlots(sortedTaskList: Array<ParagraphWithDuration>, tmb: TimeBlocksWithMap, config: { [key: string]: any }): TimeBlocksWithMap {
  ...
  for (let t = 0; t < sortedTaskList.length; t++) {
    ...
    if (newBlockList && newBlockList.length) {
      ...
      let scheduledMins = 0
      // @ts-ignore - flow doesn't like .length below but it is safe
...
export const addDurationToTasks = (tasks: Array<SortableParagraphSubset>, config: { [key: string]: any }): Array<ParagraphWithDuration> => {
  const dTasks = tasks.map((t) => {
    // @ts-ignore - Flow doesn't like spreading interfaces
