...
import { getParaAndAllChildren, isAChildPara } from '@np/helpers/blocks'
import { stringListOrArrayToArray } from '@np/helpers/dataManipulation'
import { getAPIDateStrFromDisplayDateStr, getTimeStringFromHM, getTodaysDateHyphenated, includesScheduledFutureDate } from '@np/helpers/dateTime'
import { clo, clof, JSP, logDebug, logError, logInfo, logTimer, logWarn } from '@np/helpers/dev'
import { createRunPluginCallbackUrl, displayTitle } from '@np/helpers/general'
import { sendToHTMLWindow, getGlobalSharedData } from '@np/helpers/HTMLView'
import { filterOutParasInExcludeFolders, getNoteByFilename, isNoteFromAllowedFolder, pastCalendarNotes, projectNotesFromFilteredFolders } from '@np/helpers/note'
import { getSettingFromAnotherPlugin } from '@np/helpers/NPConfiguration'
import { getReferencedParagraphs } from '@np/helpers/NPnote'
import { findEndOfActivePartOfNote, findHeadingStartsWith, findStartOfActivePartOfNote, isTermInURL, parasToText, smartPrependPara } from '@np/helpers/paragraph'
import { findParaFromStringAndFilename } from '@np/helpers/NPParagraph'
import { caseInsensitiveSubstringIncludes } from '@np/helpers/search'
import { getNumericPriorityFromPara, sortListBy } from '@np/helpers/sorting'
import { removeDateTagsAndToday } from '@np/helpers/stringTransforms'
import { eliminateDuplicateSyncedParagraphs } from '@np/helpers/syncedCopies'
import { getStartTimeObjFromParaContent, getTimeBlockString, isTypeThatCanHaveATimeBlock, RE_TIMEBLOCK_IN_LINE } from '@np/helpers/timeblocks'
import { chooseHeading, chooseNote, displayTitleWithRelDate } from '@np/helpers/userInput'
import { isOpen, isOpenTask, isOpenNotScheduled, removeDuplicates } from '@np/helpers/utils'
...
export async function getRelevantOverdueTasks(dashboardSettings: TDashboardSettings, yesterdaysParas: Array<TParagraph>): Promise<Array<TParagraph>> {
  try {
    ...
    // @ts-ignore(incompatible-call) returns ReadonlyArray type
...
export function getSectionItemObject(id: string, p: TParagraph | TParagraphForDashboard | null = null, theType?: TItemType): TSectionItem {
  // @ts-ignore - we are not using all the types in TParagraph
