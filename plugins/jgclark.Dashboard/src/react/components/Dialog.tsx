 ...
 * @return {?React.ReactNode} Renderable React node or null.
 */
const Dialog = ({ isOpen, onClose, isTask, details }: Props): React.ReactNode => {
...
function setPositionForDialog(thisOS: string, dialogWidth: number, dialogHeight: number, dialog: HTMLElement, event: TClickPosition) {
  ...
  const winWidth = window.visualViewport?.width ?? 0
  const winHeight = window.visualViewport?.height ?? 0
