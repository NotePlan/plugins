...
import { clo, logDebug, logError, logInfo, logWarn, timer } from '@np/helpers/dev'
...
import { isTermInMarkdownPath, isTermInURL } from '@np/helpers/paragraph'
import { trimAndHighlightTermInLine } from '@np/helpers/search'
...
export async function getSearchSettings(): Promise<any> {
  ...
  try {
    ...
    const v2Config: SearchConfig = await DataStore.loadJSON(`../${pluginID}/settings.json`) as any
 ...
 * @tests in jest file
 */
export function differenceByPropVal<T extends {}> (
  arr: ReadonlyArray<T>,
 ...
 * @test in jest file
 */
export function compareObjects<T extends {}>(o1: T, o2: T): boolean {
  for (const p in o1) {
 ...
 * @tests in jest file
 */
export function differenceByObjectEquality<T extends { }> (
  arr: ReadonlyArray<T>,
  exclude: ReadonlyArray<T>
) {
  return arr.filter(
    (a) => !exclude.find((b) => compareObjects(b, a))
  )
}
    ...
    if (resultParas.length > 0) {
      ...
      let resultFieldSets: Array<reducedFieldSet> = resultParas.map((p) => {
        ...
        const fieldSet = {
          ...
          title: displayTitle(note as any),
          type: p.type,
          content: p.content,
  ...
  try {
    ...
    if (linesToSync.length > 0) {
      for (const lineDetails of linesToSync) {
        ...
        const [thisIndex, thisLine, thisNote, thisPara, _thisType] = lineDetails
