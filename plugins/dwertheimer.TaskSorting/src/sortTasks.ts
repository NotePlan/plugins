...
import { chooseOption, chooseHeading, showMessage } from '@np/helpers/userInput'
import { getTagParamsFromString } from '@np/helpers/general'
import { removeHeadingFromNote, getBlockUnderHeading } from '@np/helpers/NPParagraph'
import { sortListBy, getTasksByType, TASK_TYPES, type ParagraphsGroupedByType } from '@np/helpers/sorting'
import { logDebug, logError, clo, JSP } from '@np/helpers/dev'
...
function insertTodos(note: CoreNoteFields, todos, heading = '', separator = '', subHeadingCategory = '', theTitle: string = '') {
  ...
  if (title !== '') {
    ...
    if (tasksToTop) {
      ...
      // @ts-ignore - calling function by name is not very Flow friendly (but it works!)
