...
import { RE_TIMEBLOCK, isTimeBlockPara, getTimeBlockString } from './timeblocks'
...
export async function writeTimeBlocksToCalendar(config: EventsConfig, note: TNote | TEditor, showLoadingProgress: boolean = false): Promise<void> {
  try {
    ...
    // @ts-ignore - Flow doesn't like note or Editor being called here. But for these purposes they should be identical
    const endOfActive = findEndOfActivePartOfNote(note)
    const timeblockParas = paragraphs.filter(
      (p) => isTimeBlockPara(p) && p.lineIndex <= endOfActive && ((p.type !== 'done' && p.type !== 'checklistDone') || config.includeCompletedTasks),
    )
