...
import { clo, JSP, logDebug, logError, logWarn } from '@np/helpers/dev'
...
export async function checkForRequiredSharedFiles(pluginJson: any): Promise<void> {
  try {
    ...
    if (typeof wantedRes === 'number' && wantedRes >= wantedFileList.length) {
      ...
      logWarn(
        `thisPluginID`,
        `plugin np.Shared is loaded 😄, but is only providing ${String(wantedRes)} out of ${String(
          wantedFileList.length,
        )} wanted files, so expect issues with display or functionality 😳`,
      )
    } else if (wantedRes) {
      ...
      const res = await showMessageYesNo(
        `It looks like you haven't installed the '${sharedPluginName}' plugin, which is required for the plugin to operate properly. Would you like me to install it for you?`,
        ['Yes', 'No'],
        'Plugin problem',
      )
      if (res === 'Yes') {
