...
- Fixing types (e.g. number-vs-string): `onUpdateOrInstall()` already normalises some number settings; add similar logic there if needed for the new key.
...
3. **Defaults:** The default from `dashboardSettingDefs` / `dashboardFilterDefs` is used by `getDashboardSettingsDefaults()` (in `src/dashboardHelpers.js`) and by `src/react/support/settingsHelpers.js` (`dashboardSettingsDefaults`). So adding a default there is enough for (b) for normal “merge at runtime” behaviour.
4. **Numbers:** If the setting is numeric and might be stored as a string, ensure `src/dashboardHelpers.js` `getDashboardSettings()` (and optionally `src/index.js` `onUpdateOrInstall()`) normalises it, consistent with existing number settings.
...
## Derived / coupled settings

Some settings must stay in sync when another setting changes (e.g. turning `treatTopPriorityAsWins` off also turns off `showWinsSection` as it is a "synthetic section"). Do **not** wire this in individual UI components.

1. Add a rule to `DASHBOARD_DERIVED_SETTING_RULES` in `src/dashboardSettings.js` (each rule is `{ apply(prior, next) => next }`; use `prior` for change-sensitive coupling).
2. Rules run automatically in:
   - `applyDerivedDashboardSettings()` — called from `dashboardSettingsReducer` on every `UPDATE_DASHBOARD_SETTINGS` (immediate React UI).
   - `prepareDashboardSettingsForSave()` in `src/dashboardSettingsClean.js` — called from plugin save paths (`doSaveDashboardSettingsFromBridge`, perspective save handlers, `setSetting` / `setSettings`).

For bulk perspective edits, `preparePerspectiveSettingsForSave()` applies the same pipeline per def.
