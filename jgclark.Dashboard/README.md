...
- Hide sections with nothing left to do?: When on, enabled sections with no open items are hidden after a refresh (or on first load). Completing the last item in a section still shows the usual "all done" message until the next refresh. If nothing is left to display (including after completing the last item), an info toast explains that your current settings and filters mean there are no items to show. (Default: off)
