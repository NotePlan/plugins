...
// Last updated 2025-04-22 for v2.2.2 by @jgclark
//--------------------------------------------------------------------------
...
function ItemNoteLink({ item }: Props): React$Node {
  ...
  logDebug(`ItemNoteLink`, `ItemNoteLink for item.itemFilename:${filename} noteTitle:${noteTitle}`)
  ...
  const iconToUse = item.para?.isTeamspace ? 'fa-regular fa-screen-users' : 'fa-regular fa-file-lines'
  const linkClass = item.para?.isTeamspace ? 'teamspaceName' : 'noteTitle'
  const linkStyle = item.para?.isTeamspace ? 'teamspaceName' : 'folderName'
    ...
    return (
      <TooltipOnKeyPress
        ...
        <a className={`${linkClass} ${linkStyle} sectionItem`} onClick={handleLinkClick}>
          <i className={`${iconToUse} pad-right`}></i>
          {noteTitle}
        </a>
      </TooltipOnKeyPress>
    )
