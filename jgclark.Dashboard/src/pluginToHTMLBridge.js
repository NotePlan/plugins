...
export async function bridgeClickDashboardItem(data: MessageDataObject) {
  try {
    ...
    switch (actionType) {
      ...
      case 'convertReminderToTask': {
        result = await doConvertReminderToTask(data)
        break
      }
      case 'cancelTask': {
...
async function processActionOnReturn(handlerResultIn: TBridgeClickHandlerResult, data: MessageDataObject) {
  try {
    ...
    if (actionsOnSuccess.includes('CLOSE_UNNEEDED_SECTIONS')) {
      ...
      // Remove synthetic sections, which are React-only and should not persist in pluginData. 
      sections = sections.filter((section) => !SYNTHETIC_SECTION_CODES.includes(section.sectionCode))
      // Also sync TAG sections with current settings.
      sections = syncTagSectionsWithSettings(sections, config)
      // Note: No addNewTagSections() call needed here:
      // CLOSE_UNNEEDED_SECTIONS only removes/hides stale rows in pluginData; new TAG rows are generated by refresh flows.

      reactWindowData.pluginData.sections = sections
      // `getListOfEnabledSections` is a section-code list (not section-instance list), 
      // so keep synthetic WINS in enabledForClose for accurate client-only section logging when Wins is enabled.
