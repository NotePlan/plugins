...
- dev: Document why `getCurrentlyAllowedFolders` and `getAllowedFoldersInCurrentPerspective` stay separate (circular dependency).
...
- dev: move shared Reminders list/priority/date/time/sort helpers to `@helpers/NPReminders` for reuse by other plugins.
...
- dev: Removed obsolete `refreshAllOnChange` setting flag; section refresh after settings changes is owned by `planSectionRefreshAfterDashboardSettingsChange`.
...
- fix: Overdue / Tag sort order `earliest`, `due date`, and `most recent` now stick. Dev: useSectionSortAndFilter no longer re-sorts those sections by priority after the plugin has ordered them.
- fix: The 'Last Week' section wasn't being handled properly when "Hide duplicates" setting was enabled.
