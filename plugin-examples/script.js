...
function cachedFolders() {
    var folders = DataStore.folders // All folders as [String] array
    console.log("Folders:\n" + folders)
}

// example5
function calendarNotes() {
    var notes = DataStore.calendarNotes // All calendar notes as [String] array
...
function projectNotes() {
    var notes = DataStore.projectNotes // All project notes as [String] array
...
async function createEvent() {
    ...
    if(dates.length >= 0) {
        ...
        var event = CalendarItem.create(title, start, end, "event", false)
...
function getAllParagraphsFromNote() {
    let today = DataStore.calendarNoteByDate(new Date())
