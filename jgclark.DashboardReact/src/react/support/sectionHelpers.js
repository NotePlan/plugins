// @flow
import { type TSection, type SectionCode } from '../../types.js'
 ...
 * @param {Array<SectionCode>} useFirst - Priority order of sectionType names to determine retention priority
...
export function removeDuplicates(_sections: Array<TSection>, paraMatcherFields: Array<string>, useFirst: Array<SectionCode>): Array<TSection> {
  ...
  const orderedSections = useFirst.map((st) => sections.find((section) => section.sectionType === st)).filter(Boolean)

  // Include sections not listed in useFirst at the end of the array
  orderedSections.push(...sections.filter((section) => !useFirst.includes(section.sectionType)))

  // Process each section (but not if it's a "PROJ" section)
  orderedSections.forEach((section) => {
    section.sectionItems =
      section.sectionType === 'PROJ'
        ? section.sectionItems
