...
- **Other reads**: `[buildAllProjectTags](jgclark.Reviews/src/projectClass.js)` ~816–821; `[gatherAnyNextActionContent](jgclark.Reviews/src/projectClass.js)` ~1228–1232 — both use the same key for hashtag content.
...
7. **Detection and sequential / paused**
  - `[isProjectNoteIsMarkedSequential](jgclark.Reviews/src/reviewHelpers.js)` and `[gatherAnyNextActionContent](jgclark.Reviews/src/projectClass.js)`: continue reading the **hashtag** key (`projectMetadataFrontmatterKey`); ensure no code path still expects date mentions there.
