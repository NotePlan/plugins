...
async function renderAndInsertTemplate(
  ...
  try {
    ...
    const resultingTextContent = (await DataStore.invokePluginCommandByName('renderTemplate', 'np.Templating', [templateTitle])).trim()
    // If no resulting text, or just a newline, then stop early, to try to avoid race conditions for tag-only templates.
    if (resultingTextContent == null || resultingTextContent === '' || resultingTextContent === '\n') {
      logDebug('renderAndInsertTemplate', `No resulting text from running Template '${templateTitle}'. Stopping.`)
      return
    } else {
      logDebug('renderAndInsertTemplate', `Successfully rendered Template '${templateTitle}' -> ${resultingTextContent.length} characters.`)
    }
