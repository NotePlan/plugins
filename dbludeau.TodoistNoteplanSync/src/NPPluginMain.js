...
export async function syncEverything() {
    ...
    console.log(`Folder for everything notes: ${ setup.folder })`)
    ...
    if (folders.length === 0) {
        try {
            ...
            console.log(`New folder has been created (${  setup.folder  })`)
        } catch (error) {
            console.log(`Unable to create new folder (${  setup.folder  }) in Noteplan (${  JSON.stringify(error)  })`)
            process.exit(1)
        }
    }
...
export async function syncToday() {
    ...
    console.log(`Todays date is: ${  date_string}`)
    ...
    fetch(`${todo_api  }/tasks?filter=today`,
        {
            ...
            if (setup.header !== "") {
                ...
                if (!existingHeader.exists) {
                    console.log(`Creating Heading: ${  setup.header }`)
            ...
            tasks.forEach((task) => {
                if (!existing.includes(task.id)) {
                    ...
                    note.addTodoBelowHeadingTitle(fortmatted, setup.header, true)
                }
            })
        }).error((error) => {
            // if the user has set a heading, create that
            console.log(`Error getting today tasks from Todoist (${  JSON.stringify(error)  })`)
        })
}
...
function getExistingNote(project_name) {
    ...
    if (existing_notes.length > 0) {
        ...
        console.log(`Creating note: ${  project_name  } in: ${  setup.folder}`)
        try {
...
async function getTodoistProjects() {
    try {
        const response = await fetch(`${todo_api  }/projects`,
            {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${  setup.token}`,
