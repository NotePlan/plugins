...
export async function getDailyQuote(quoteParams: mixed, config: { [string]: ?mixed }): Promise<string> {
  ...
  if (quoteParams != null) {
    await showMessage('\tInfo: {{quote()}} tag parameters are currently not supported')
  }

  const quoteConfig: any = config.quote ?? null
  if (quoteConfig == null) {
    console.log(`\tInfo: Missing 'quote' config in Templates/_configuration note`)
  }
  ...
  if (pref_mode === 'readwise') {
    ...
    if (response != null) {
      ...
      const msg = `\tError in Quote lookup to ${API}. Please check your _configuration note.`
      console.log(msg)
      return msg
    }
  } else {
