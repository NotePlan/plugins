...
headings.forEach((h) => {
  ...
  // Replace parentheses with encoded versions (%28,%29)
  const encLink = `noteplan://x-callback-url/openNote?noteTitle=${encodeURIComponent(Editor.title)}%23${encodeURIComponent(h.content).replace(/\(/g,'%28').replace(/\)/g,'%29')}`;
