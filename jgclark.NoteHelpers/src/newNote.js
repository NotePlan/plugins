...
// Last updated 2025-07-14 for 1.2.0, @jgclark (originally @dwertheimer)
//-----------------------------------------------------------------------------
...
export async function newNote(): Promise<void> {
  try {
    ...
    logError(pluginJson, `newNote: ${err.message}`)
  }
}

/**
 * Create a new note from the clipboard contents.
 * @author @jgclark
 */
export async function newNoteFromClipboard(): Promise<void> {
  try {
    ...
    if (string == null || string.length === 0) {
      logWarn(pluginJson, 'newNoteFromClipboard: The clipboard was empty, so nothing to do.')
      await showMessage("The clipboard was empty, so there's nothing to do.", "OK", `New Note from Clipboard`)
      return
    }
    ...
    const existingFMTitle = (string.match(/title:\s+(.*)/))?.[1] ?? ''
    const strippedFirstLine = lines[0].replace(/^#+\s/, '')
    const titleToOffer = existingFMTitle || strippedFirstLine || ''
    ...
    if (typeof title === 'string') {
      ...
      if (title !== uniqueTitle) {
        const res = await showMessageYesNo(`That title already exists. Use "${uniqueTitle}" instead?`, ['Yes', 'Cancel'], `New Note from Clipboard`, false)
        if (res === 'Yes') {
          title = uniqueTitle
        } else {
          logWarn(pluginJson, `newNoteFromClipboard: User cancelled the operation.`)
          return
        }
      }
      const currentFolder = await chooseFolder('Select folder to add note in:', false, true)  // don't include @Archive as an option, but do allow creation of a new folder
    ...
    logError(pluginJson, `newNoteFromClipboard: ${err.message}`)
  }
}
...
export async function newNoteFromSelection(): Promise<void> {
  try {
    ...
    if (note != null && selectedLinesText.length && selectedText !== '') {
      ...
      const strippedFirstLine = selectedParagraphs[0].content.replace(/^#+\s/, '')
      ...
      // If user selected the first line, then remove it from the body content
        if (title === strippedFirstLine && selectedParagraphs[0].type === 'title') {
    ...
    logError(pluginJson, `newNoteFromSelection: ${err.message}`)
  }
}

/**
 * Create a new note from the current note, with a title that includes the current date.
...
export async function smartDuplicateRegularNote(): Promise<void> {
  try {
    ...
    logError(pluginJson, `smartDuplicateRegularNote: ${err.message}`)
  }
}