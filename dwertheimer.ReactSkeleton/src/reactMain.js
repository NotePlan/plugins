 ...
 */
export function getDataObjectForReactView(): PassedData {
  const startTime = new Date()
  // get whatever pluginData you want the React window to start with and include it in the object below. This all gets passed to the React window
  const pluginData = getData()
  // make sure to change dwertheimer.ReactSkeleton to your plugin name below
 ...
 * @author @dwertheimer
 */
export async function testReactWindow() {
  try {
...
async function handleSubmitButtonClick(data, reactWindowData) {
  ...
  await sendBannerMessage(`Plugin received an actionType: "onSubmitClick" command with data:\n${JSON.stringify(data)}`)
...
export async function onMessageFromHTMLView(actionType: string, data: any): Promise<any> {
  try {
    ...
    let reactWindowData = await getGlobalSharedData() // get the current data from the React Window
    ...
    switch (actionType) {
      ...
      case 'onSubmitClick':
        ...
        break
      default:
        await sendBannerMessage(`Plugin received an unknown actionType: "${actionType}" command with data:\n${JSON.stringify(data)}`)
        break
    }
    if (reactWindowData) {
